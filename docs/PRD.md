...
实现见 `src/algorithms/cards.ts`。同一张单词卡随着熟练度更换提问方式，而不是每次都问同一个问题：

| 形态 | 条件 | 问什么 |
|------|------|--------|
| `context` | `repetitions === 0` | 保存时那句话，目标词标出，"What does it mean here?" —— 认出意思 |
| `cloze` | `repetitions >= 1` 且目标词可定位 | 同一句话挖空，释义作提示 —— 从意思反推出词 |
| `fresh` | `repetitions >= 3` 且义项例句多于一条 | 换一句这张卡没用过的例句 —— 考的是词，不是那句话 |
| `bare` | 该义项没有例句 | 退回光词头（schema 2 之前的老词条） |

只要手上有句子就不会光考词：脱离语境回忆单词，正是本产品想替换掉的背单词表习惯。

`fresh` 目前只能在该义项已有的例句之间轮换（`1 + ((reps - 3) % (examples.length - 1))`）。
真正"没见过的新句子"有两个来源，都还没做：给义项按需生成额外例句，以及语境采集（用户自己遇到的句子）。

### 6.3 词形还原（Lemmatization）
