...
Lexi 是一款专注于"查得到、记得住"的英语学习工具。区别于传统词典 App 只解决"查"的问题，Lexi 把用户每一次查词和翻译都自动沉淀为可复习的素材，通过 SM-2 间隔重复算法帮助用户真正掌握。
